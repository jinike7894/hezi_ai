...
class H5 extends BaseController
{
    ...
    public function index($category='sy',$pag=1,$channel)
    {
        ...
        $banners = $this->Products->field('id,img,name,androidurl,is_apk,is_browser,iosurl,cid')->where(array('status'=>1,'is_banner'=>1))->order('sort asc')->limit(6)->select();
        ...
        $limit = 100;
        $pid = 76;
        $kanpiantongcheng =$this->Products->getproducts($page,$limit,$pid);
        foreach ($kanpiantongcheng as $key => &$item) {
            $item['url'] = $system == 'Android' ? cjqd($channel, $item['androidurl']) : (cjqd($channel, $item['iosurl']) ?: cjqd($channel, $item['androidurl']));
            if (strpos($item['img'], 'http') === false) {
                $item['img'] = str_ireplace(['.png','.jpg','.gif','.jpeg','.webp'],'.js',$item['img']);
            }
        ...
        $this->app->view->assign('kanpiantongcheng', $kanpiantongcheng);
	 ...
	 public function index1($channel)
    {
        ...
        $banners = $this->Products->field('id,img,name,androidurl,is_apk,is_browser,iosurl,cid')->where(array('status'=>1,'is_banner'=>1))->order('sort asc')->limit(6)->cache(600)->select();
    ...
    public function detail($channel){
        $channelTemp = explode('_', $channel);
        $arr_param['channelCode']  = $channelTemp[0];
		$arr_param['subid'] = intval($channelTemp[1] ?? 0);
		View::assign('linkId',$arr_param['subid']);
		View::assign('channel', $channel);
		View::assign('channelCode', $arr_param['channelCode']);
		View::assign('key', 'rinimei');
        $this->Ptype = new \app\common\model\Ptype();
	    $id=input('param.id');
	    $channel = intval($channel);
	    if($channel == 0){$channel = 1;}
	    $list = $this->Products->field('id,img,name,slogan,androidurl,iosurl,is_apk,pid,is_browser,downnum,pics,content,cid')->where(array('status'=>1,'id'=>$id))->cache(600)->find();
	    $arr= array();
	    if(!empty($list))
	    {
	        ...
	        $arr['data']['list'] = $list;
            $hotList = $this->Products->field('id,img,name,androidurl,cid')->where(array('status'=>1,'is_banner'=>0,'ad_position'=>0))->order('downnum desc')->limit(5)->cache(600)->select();
	        foreach ($hotList as &$item) {
                $item['name'] = str_replace('{city}', str_replace('市', '', $region), $item['name']);
            }
            $arr['data']['hot'] = $hotList;
            $relatedList = $this->Products->field('id,cid,img,name,androidurl,cid')->where(array('status'=>1,'is_banner'=>0,'pid'=>$list['pid'],'ad_position'=>0))->order('sort asc')->limit(5)->cache(600)->select();
	    ...
	    View::assign('base_url', $this->api_url);
	    View::assign('arr',$arr['data']);
	    return View::fetch("detail");
	   // $this->app->view("detail")->assign('arr', $arr);
	   // dump($arr);die;
    //     return jiami(json_encode($arr));
    }
     //详情页2
    public function detail2($channel){
        $channelTemp = explode('_', $channel);
        $arr_param['channelCode']  = $channelTemp[0];
		$arr_param['subid'] = intval($channelTemp[1] ?? 0);
		View::assign('linkId',$arr_param['subid']);
