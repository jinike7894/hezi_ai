...
class AiActivityRecord extends \think\Model
{
    ...
    //任务完成回调
     public static function activityFinishNotify($activityOrderNum)
    {
       $activityData=self::where(["activity_order_num"=>$activityOrderNum])->find();
       $userData=AiUser::where(["id"=>$activityData["uid"]])->find();
        try {
            Db::transaction(function () use ($activityData,$userData) {
                // 增加用户点数
                $userRes = AiUser::where(['id' => $activityData["uid"]])
                    ->inc("free_points", $activityData["points"])
                    ->update();
                if (!$userRes) {
                ...
                $pointsBillParams = [
                    ...
                    "after_points" => $userData["points"] +$userData["free_points"]+$activityData["points"],
                    "bill_type" => 0,
                    "points_type" => 1,
                    "create_time" => time(),
                    "update_time" => time(),
                ...
                $pointsBillRes=AiPointsBill::create($pointsBillParams);
                 if (!$pointsBillRes) {
    ...
    // public static function activityFinishNotify($uid,$pid)
    // {
    //     $params = [
    //         "pid" => $pid,
    //     ];
